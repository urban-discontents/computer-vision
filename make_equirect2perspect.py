...
import streetview, glob, os, os.path, sys, time, datetime, json, shutil, cv2
import numpy as np
...
def make_pers(view_number):
    for filename in os.listdir(img_dir):
        if filename.endswith('.jpg'):
            print(filename)
            equ = Equirectangular(img_dir + filename)
            img0 = equ.GetPerspective(90, view_number*90, 0, 2400, 2400) # Specify parameters(FOV, theta, phi, height, width)
            oupname = img_dir+filename+'__'+str(view_number)
            cv2.imwrite(oupname, img0, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
            print('saved img to ' + oupname)
